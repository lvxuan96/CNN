...
#pragma omp barrier
  ...
  if(Caffe::root_thread() && Caffe::mpi_count() > 1){
   ...
   // MPI_Allreduce(src, buffer_, size, MpiDataType<Dtype>::type, MPI_SUM, MPI_COMM_WORLD);
    if(caffe::mpi_rank!=0){
      for(int send=1; send<Caffe::mpi_count; send++){
        MPI_Send(src, size, MpiDataType<Dtype>::type, 0, 99, MPI_COMM_WORLD);
      }
    }
    else{
      for(int recv=1; recv<Caffe::mpi_count; recv++){
        MPI_Recv(buffer_, size, MpiDataType<Dtype>::type, recv, 99, MPI_COMM_WORLD);
        caffe_add(size, buffer_, src, src);
      }
    }
...
void ThreadLocal<Dtype>::syncNodeData() {
 ...
 // MPI_Barrier(MPI_COMM_WORLD);
 // MPI_Bcast(data_, size_, MpiDataType<Dtype>::type, 0, MPI_COMM_WORLD);
  if(Caffe:mpi_rank==0){
    for(int process_id=1; process_id<Caffe::mpi_count; process_id++)
       MPI_Send(data_, size_, MpiDataType<Dtype>::type, process_id, 99, MPI_COMM_WORLD);
    }
...
#pragma omp barrier
  ...
  if(Caffe::root_thread() && Caffe::mpi_count() > 1){
   ...
   // MPI_Barrier(MPI_COMM_WORLD);
   // MPI_Allreduce(svec.data(), buffer_, svec.size(), MpiDataType<Dtype>::type, MPI_SUM, MPI_COMM_WORLD);
   if(caffe::mpi_rank!=0){
      for(int send=1; send<Caffe::mpi_count; send++){
        MPI_Send(svec.data(), svec.size(), MpiDataType<Dtype>::type, 0, 99, MPI_COMM_WORLD);
      }
    }
    else{
      rootbuffer_= buffer_;
      for(int recv=1; recv<Caffe::mpi_count; recv++){
        MPI_Recv(buffer_, svec.size(), MpiDataType<Dtype>::type, recv, 99, MPI_COMM_WORLD);
        rootbuffer_ += buffer;
      }
    }
